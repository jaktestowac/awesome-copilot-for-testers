...
The loop decides *where* the seams go. Nothing inside it decides *whether* a change is worth cycling at all.
...
per iteration, and an agent looping on one for a feature that does not exist yet reliably concludes the *test* is broken.
