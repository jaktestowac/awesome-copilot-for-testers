...
**Variants worth running:** payload in a footnote nobody reads; payload split across two chunks that are retrieved together; payload in a document that only ranks for a rare query; payload in document _metadata_ (title, tags) rather than body; payload in a filename.
...
**The strongest structural control:** never allow a tool call to be _decided_ by retrieved content alone. If retrieval can trigger a tool, retrieval is a command channel.
...
**Test:** get a payload into the first stage (via any channel above) and check whether it survives into the second stage's instruction space. Then check the reverse direction: can a sub-agent's _report_ influence the orchestrator's next tool call?
