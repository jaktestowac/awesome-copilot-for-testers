...
This is a floor, not a finished gate: it proves *some* rationale exists on a range containing high-risk surface. It cannot tell whether the rationale is real, which is the part a human does in review. Pair it with the "restated diff" check in review, not in CI.
