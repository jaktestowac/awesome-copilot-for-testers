...
**Verification type** matters for enforcement: `signal` practices are detected from configuration, `automated` practices produce a machine verdict, and `manual-attestation` practices can only ever be _attested_ by a human - they must never be reported as auto-passed. See `attesting-manual-verification`.
