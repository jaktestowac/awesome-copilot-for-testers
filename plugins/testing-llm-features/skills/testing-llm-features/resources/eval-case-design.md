...
Layer expectations: a cheap structural check plus one substantive check per case. The structural check tells you *that* it broke; the substantive one tells you *how*.
...
- **Freeze retrieval.** For RAG, snapshot the retrieved documents into the fixture so an index change cannot silently change the test. Test the *live* index separately, as a retrieval eval.
